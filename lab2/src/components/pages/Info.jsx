import info from "../content/info.js";

export default function Info() {
  return (
    <main className="info">
      <h2>Information</h2>
      <section>
        <h3>What do we offer?</h3>
        {info.map((content, index) => (
          <p key={index}>{content}</p>
        ))}
      </section>
      <section>
        <h3>Who are the users?</h3>
        <p>
          The target audience for Waste Rewarder primarily consists of students
          and young adults who are passionate about environmental sustainability
          and are looking for innovative ways to make a positive impact. The app
          is particularly appealing to individuals who enjoy video games, as the
          rewards earned through proper waste management are claimable within
          popular video game platforms. By tapping into the gaming community,
          Waste Rewarder aims to engage a demographic that is tech-savvy and
          motivated by rewards and incentives.
        </p>
        <p>
          Additionally, the app is designed to appeal to environmentally
          conscious individuals who are seeking practical and fun ways to
          incorporate sustainable habits into their daily lives. Waste Rewarder
          provides an opportunity for users to not only contribute to a cleaner
          environment but also to be rewarded for their efforts in a way that
          aligns with their interests and hobbies. By targeting students and
          young adults who are passionate about sustainability and enjoy gaming,
          Waste Rewarder aims to create a community of environmentally conscious
          individuals who are motivated to make a difference through positive
          reinforcement and tangible rewards.
        </p>
      </section>
      <section>
        <h3>Resources</h3>
        <p>
          These resources can provide valuable information and insights into
          waste management practices, policies, and sustainable solutions that
          can be incorporated into this web app.
        </p>
        <ul>
          <li>
            <a
              href="https://www.epa.gov/waste"
              rel="noreferrer"
              target="_blank"
            >
              Environmental Protection Agency (EPA) - Waste Management
            </a>
            : The EPA provides comprehensive information on waste management
            practices, regulations, and resources for individuals and
            businesses.
          </li>
          <li>
            <a
              href="https://www.unep.org/explore-topics/waste"
              rel="noreferrer"
              target="_blank"
            >
              United Nations Environment Programme (UNEP) - Waste Management
            </a>
            : UNEP offers reports, publications, and guidelines on sustainable
            waste management practices and policies.
          </li>
          <li>
            <a
              href="https://www.waste360.com/"
              rel="noreferrer"
              target="_blank"
            >
              Waste360
            </a>
            : Waste360 is a leading information, event, and education provider
            for the waste, recycling, and organics industry. It offers news,
            articles, and resources on waste management.
          </li>
          <li>
            <a
              href="https://waste-management-world.com/"
              rel="noreferrer"
              target="_blank"
            >
              Waste Management World
            </a>
            : This online publication covers the latest news, trends, and
            technologies in waste management and recycling. It provides valuable
            insights and resources for waste management professionals.
          </li>
          <li>
            &quot;Waste Management and Sustainable Consumption: Reflections on
            Consumer Waste&quot; by Dr. Tanja Sobko: This academic article
            explores the relationship between consumer behavior and waste
            management, offering insights into sustainable consumption and waste
            reduction. Available at:{" "}
            <a
              href="https://www.mdpi.com/2071-1050/12/6/2363"
              rel="noreferrer"
              target="_blank"
            >
              https://www.mdpi.com/2071-1050/12/6/2363
            </a>
          </li>
          <li>
            &quot;Waste Management Practices: Municipal, Hazardous, and
            Industrial&quot; by John Pichtel: This book provides an in-depth
            overview of waste management practices, including municipal,
            hazardous, and industrial waste, offering valuable insights into
            waste management strategies. Available at:{" "}
            <a
              href="https://www.springer.com/gp/book/9783319749567"
              rel="noreferrer"
              target="_blank"
            >
              https://www.springer.com/gp/book/9783319749567
            </a>
          </li>
        </ul>
      </section>
    </main>
  );
}
